...
* 브리지 클래스(Bridge Stream) 는 바이트 기반 스트림을 문자 기반 스트림으로 변환하거나 그 반대로 변환해 주는 “다리” 역할의 I/O 클래스로, 대표적인 예로 InputStreamReader와, OutputStreamWriter가 있음.

- 역할 : 1 byte 단위로 흘러오는 데이터(네트워크·파일·System.in/out 등)를 2 byte 유니코드 문자 단위로 디코딩/인코딩해 줍니다
...
- 설계도를 바탕으로 소프트웨어 세계 **구현된 구체적인 실체**
    ...
    + “인스턴스”는 일반적인 개념이 실제로 구현된 구체적인 예를 뜻하는 단어
